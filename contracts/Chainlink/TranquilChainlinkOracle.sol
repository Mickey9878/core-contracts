...
contract TranquilChainlinkOracle is PriceOracle {
    ...
    function getUnderlyingPrice(TqToken tqToken) public view returns (uint) {
        string memory symbol = tqToken.symbol();
        if (compareStrings(symbol, "tqONE")) {
            return getChainlinkPrice(getFeed(symbol));
        } else {
            return getPrice(tqToken);
        }
    }

    function getPrice(TqToken tqToken) internal view returns (uint price) {
        EIP20Interface token = EIP20Interface(TqErc20(address(tqToken)).underlying());
    ...
    function setUnderlyingPrice(TqToken tqToken, uint underlyingPriceMantissa) external onlyAdmin() {
        address asset = address(TqErc20(address(tqToken)).underlying());
